...
let {complie,deploy} = require('./deploy')
...
var fs   = require('fs');
var abi  = fs.readFileSync('safe.abi');
var addr = fs.readFileSync('safe.addr');

async function initSAFE(sol)
{
	if(abi == null || addr == null)
	{
		let {bytecode, abi} = complie(sol)
		if(abi == null) {
  ...
  constructor(abi,addr) 
	...
	async getinfo()
	{
		try
		{
			...
			console.log("accounts[1]:", await this.balanceOf(accounts[1]))
			console.log("accounts[2]:", await this.balanceOf(accounts[2]))
